import {TTrinhTham, TVanHoc, TTamLy, TThieuNhi} from './data.js'
import {trinhtham_arr, vanhoc_arr, tamly_arr, thieunhi_arr} from './data.js'

const hien1TrinhTham = ( trinhtham: TTrinhTham) => {
     return `<div>
     <h3> ${trinhtham .ten} </h4>
    ...
    export const showlisttrinhtham = () => {
      const html_arr =trinhtham_arr.map(hien1TrinhTham); // call back
...
const hien1VanHoc = ( vanhoc: TVanHoc) => {
    return `<div>
    <h3> ${vanhoc .ten} </h4>
    <img src="${vanhoc .hinh}">
   ...
   export const showlistvanhoc = () => {
     const html_arr =vanhoc_arr.map(hien1VanHoc); // call back
     return html_arr.join("");
    let str=``;
...
const hien1TamLy = ( tamly: TTamLy) => {
   ...
   export const showlisttamly = () => {
     const html_arr =tamly_arr.map(hien1TamLy); // call back
     return html_arr.join("");
    let str=``;
...
const hien1ThieuNhi = ( thieunhi: TThieuNhi) => {
    return `<div>
    <h3> ${thieunhi .ten} </h4>
   ...
   export const showlistthieunhi = () => {
     const html_arr =thieunhi_arr.map(hien1ThieuNhi); // call back
     return html_arr.join("");
    let str=``;
